...
# use pool4 results to conv2d with 1X1X21 to get 32X32X21
...
# Perform the upsampling  2X once  16X16X21-->32X32X21
...
upsampled_logits_2X_1 = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2,
                                          output_shape=tf.shape(aux_logits_16s),
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')

upsampled_logits_total_1 = upsampled_logits_2X_1 + aux_logits_16s

# use pool3 results to conv2d with 1X1X21 to get 64X64X21
pool3_feature = end_points['vgg_16/pool3']
with tf.variable_scope('vgg_16/fc8'):
    aux_logits_64s = slim.conv2d(pool3_feature, number_of_classes, [1, 1],
                                 activation_fn=None,
                                 weights_initializer=tf.zeros_initializer,
                                 scope='conv_pool3')

# Perform the upsampling  2X twice  32X32X21-->64X64X21
upsampled_logits_2X_2 = tf.nn.conv2d_transpose(upsampled_logits_total_1, upsample_filter_tensor_x2,
                                          output_shape=tf.shape(aux_logits_64s),
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')


upsampled_logits_total_2 = upsampled_logits_2X_2 + aux_logits_64s
...
upsampled_logits = tf.nn.conv2d_transpose(upsampled_logits_total_2, upsample_filter_tensor_x8,
